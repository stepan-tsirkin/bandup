...
### you need to install packages `bandupy` and `irrep` from pip
...
try:
    ...
    try:
        ...
        try: 
            ...
            bands=bandupy.BandStructure(code="espresso", prefix="bulk_Si")
            ####  this is a shortcut to   bands=irrep.bandstructure.BandStructure(...)
            ####   examples for other codes are:
            ####  VASP:
            ####      bands=bandupy.BandStructure(fWAV='WAVECAR',fPOS='POSCAR',spinor=False,code='vasp')
            ####  Abinit:
            ####      bands=bandupy.BandStructure(fWFK='mysystem_WFK',code='abinit')
            ####  Files preparedfor Wannier90 (.eig, .win, UNK* ):
            ####      bands=bandupy.BandStructure(prefix='nbcosb',code='wannier90')
